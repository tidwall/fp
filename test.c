...
#define test(fmt, input, expected) { \
    char buf[256]; \
    size_t n = fp_dtoa((input), (fmt), buf, sizeof(buf)); \
    ...
    assert(n == strlen(buf)); \
    double x; \
    assert(fp_atod(buf, n, &x)); \
    char buf2[256]; \
    size_t n2 = fp_dtoa(x, (fmt), buf2, sizeof(buf)); \
    assert(n == n2); \
...
int main(void) {
    ...
    char buf[32];
    size_t n1 = fp_dtoa(-112.89123883, 'f', buf, sizeof(buf));
    assert(strcmp(buf, "-112.89123883") == 0);
    size_t n2 = fp_dtoa(-112.89123883, 'f', NULL, 0);
    assert(n1 == n2);
    size_t n3 = fp_dtoa(-112.89123883, 'f', buf, 5);
    assert(n3 == n2);
    assert(strcmp(buf, "-112") == 0);
    fp_dtoa(-112.89123883, 'f', buf, 1);
    assert(strcmp(buf, "") == 0);
    fp_dtoa(-112.89123883, 'f', buf, 2);
    assert(strcmp(buf, "-") == 0);
    fp_dtoa(-112.89123883, 'f', buf, 6);
    assert(strcmp(buf, "-112.") == 0);
    test('g', -0.01, "-0.01");
    test('f', 5000000000000000000.0, "5000000000000000000");
    ...
    printf("\n");
    printf("For the following floating point strings:\n");
    printf("\n");
    ...
    for (size_t i = 0; i < list_len; i++) {
        ...
        double x;
        fp_atod(list+i, info.len, &x);
        char dst[32];
        fp_dtoa(x, 'g', dst, sizeof(dst));
        printf("| %-19s | %-12g |\n", dst, x);
        i += info.len;
    }
    printf("+---------------------+--------------+\n");
